...
def render_admin_panel_tab(tab):
    """Render the admin panel tab content"""
    password = ui.render_admin_panel_tab()
    ...
    if hashlib.sha256(password.encode()).hexdigest() == admin_pass_hash:
        ...
        with col1:
            if st.button("🧹 Clear All Detections", use_container_width=True):
                # Clear all user detection directories
                for user_dir in os.listdir("detections"):
                    user_path = os.path.join("detections", user_dir)
                    if os.path.isdir(user_path):
                        for f in os.listdir(user_path):
                            os.remove(os.path.join(user_path, f))
                st.info("All saved detections cleared for all users.")
        ...
        # Collision area configuration
        st.markdown('<div class="card">', unsafe_allow_html=True)
        st.subheader("Collision Area Configuration")

        st.checkbox("Default Collision Area Enabled", value=True, key="default_collision_enabled")

        col1, col2 = st.columns(2)
        with col1:
            st.number_input("Default X1", 0, 640, 200, key="default_x1")
            st.number_input("Default Y1", 0, 480, 300, key="default_y1")
        with col2:
            st.number_input("Default X2", 0, 640, 440, key="default_x2")
            st.number_input("Default Y2", 0, 480, 450, key="default_y2")

        if st.button("Save Default Collision Area"):
            st.session_state.collision_area = {
                "x1": st.session_state.default_x1,
                "y1": st.session_state.default_y1,
                "x2": st.session_state.default_x2,
                "y2": st.session_state.default_y2,
                "enabled": st.session_state.default_collision_enabled
            }
            st.success("Default collision area updated.")
        st.markdown('</div>', unsafe_allow_html=True)
        ...
        st.subheader("Trajectory Prediction Settings")
        st.slider("Prediction Time Steps", 5, 20, 10, 1,
                  help="Number of future frames to predict for collision detection")
        st.slider("Velocity Smoothing Factor", 0.1, 0.9, 0.7, 0.1,
                  help="Higher values make trajectory predictions more stable but less responsive")
